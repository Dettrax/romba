...
def compute_v(
    ...
    if hparams.mamba_block_non_ssm or hparams.mamba_block_ssm:
        try:
            ...
            # Get the total output size of the current layer
            total_output_size = cur_output.size(-1)  # 3352 in this case
            hidden_size = 768  # Base size of one segment

            # Compute the number of full segments and remaining size
            num_full_segments = total_output_size // hidden_size  # 4 full segments
            remaining_size = total_output_size % hidden_size  # Remainder: 3352 % 768 = 280

            # Construct split sizes
            split_sizes = [hidden_size] * num_full_segments  # [768, 768, 768, 768]
            if remaining_size > 0:
                split_sizes.append(remaining_size)  # Add the remaining 280 as the last segment

            # Split the cur_output tensor
            segments = cur_output.split(split_sizes, dim=-1)

            # Example: Assign first two segments to SSM and the rest to non-SSM
            ssm_input = torch.cat(segments[:2], dim=-1)  # First 2 segments
            non_ssm_input = torch.cat(segments[2:], dim=-1)  # Remaining segments

            # Use appropriate part based on the configuration
            if hparams.mamba_block_ssm:
                cur_output = ssm_input
            else:
                cur_output = non_ssm_input
    ...
    try:
        ...
        cur_output = cur_output[:target.size(0)]
        right_vector = (target - cur_output) / torch.dot(cur_input, left_vector)
