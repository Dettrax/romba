...
#from src.utils.typing import Mamba #, Mamba2LMHeadModel
...
# Define Mamba as the correct type
Mamba = Mamba2ForCausalLM
...
class ModelandTokenizer:
    def __init__(
        ...
        if "mamba" in model_path.lower():
            # model = Mamba.from_pretrained(model_path).to(torch_dtype).to(device)
    ...
    def parse_config(self, model_config=None) -> None:
        ...
        # if isinstance(self.model, Mamba2LMHeadModel):
        #     self.model.config = self.model.args
        #     self.model.config.hidden_size = self.model.config.d_model
...
def determine_embedding_layer_path(model: ModelandTokenizer | Model) -> str:
    ...
    elif isinstance(model, Mamba) or isinstance(model, Mamba2ForCausalLM):
